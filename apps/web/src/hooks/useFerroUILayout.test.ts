...
import { useFerroUILayout } from './useFerroUILayout.js';

describe('useFerroUILayout', () => {
  beforeEach(() => {
  ...
  it('handles successful streaming', async () => {
    ...
    const { result, unmount } = renderHook(() => useFerroUILayout({ url: '/test' }));
  ...
  it('handles parsing error silently', async () => {
    ...
    const { result } = renderHook(() => useFerroUILayout({ url: '/test' }));
