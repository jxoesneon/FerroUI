...
describe('OllamaProvider', () => {
  ...
  it('should have correct id and context window', () => {
    ...
    expect(provider.contextWindowTokens).toBe(4096);
  });
  ...
  it('should stream processPrompt correctly', async () => {
    ...
    expect(chunks.join('')).toContain('simulated local response from Ollama for: test prompt');
    expect(response.content).toBe('This is a simulated local response from Ollama for: test prompt');
